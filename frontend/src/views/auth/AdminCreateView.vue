  ...
  <div class="admin-create-container">
    <div class="card">
      <div class="card-header">
        <h2>{{ isSetupMode ? 'Configuration de l\'administrateur initial' : 'Créer un administrateur' }}</h2>
      </div>
      <div class="card-body">
...
export default {
  ...
  setup() {
    ...
    const isSetupMode = computed(() => route.name === 'admin-setup')
    
    onMounted(() => {
      // Vérifier si l'utilisateur est authentifié et admin pour la création d'admin
      if (!isSetupMode.value && (!authService.isAuthenticated() || !authService.isAdmin())) {
        router.push('/login')
      }
    })
    ...
    const validateForm = () => {
      ...
      if (isSetupMode.value && !setupKey.value) {
    ...
    const submitForm = async () => {
      ...
      try {
        ...
        successMessage.value = isSetupMode.value 
          ? 'Administrateur initial créé avec succès. Vous pouvez maintenant vous connecter.' 
        ...
        // Rediriger vers la page de connexion après la configuration initiale
        if (isSetupMode.value) {
          setTimeout(() => {
            router.push('/login')
          }, 3000)
        ...
        errorMessage.value = error.message || 'Une erreur est survenue lors de la création de l\'administrateur'
      } finally {
